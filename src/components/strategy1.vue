...
<div>
    ...
    <div>
        大于<input type="text" v-model="zs">为趋势，
        其余为回撤
    ...
    <p>平均趋势为{{mathQS}} 趋势次数{{mathQSs.length}}</p>
    <p>平均回撤为{{mathHc}} 回撤出现次数{{mathHcs.length}}</p>
    <!-- <div>
    ...
    <!-- <div class="box">
        <p>波段数值分布</p>
        <div class="dot" v-for="(item,idx) in hcs"
         :key="idx" 
        :style="'left:'+idx+'px;top:'+item+'px'" 
        >
        {{item}}
        </div>
    </div> -->
...
export default {
  ...
  computed: {
    ...
    mathQS() {
      ...
      console.log(sum);
      return Math.floor(sum / this.mathQSs.length);
    }
  },
  mounted() {
    this.init();
